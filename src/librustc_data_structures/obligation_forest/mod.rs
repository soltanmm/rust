...
// FIXME make the `obligation` member a `Cow<O>` instead of an `O` to avoid cloning so much
...
enum NodeState<O> {
    ...
    /// Popped then was committed to Snapshot 0 (the base).
    Popped,
}
...
impl<O: Debug + Clone> ObligationForest<O> {
        ...
        where E: Debug, F: FnMut(&mut O, Backtrace<O>) -> Result<Option<Vec<O>>, E>
    {
        ...
        for index in 0..self.nodes.len() {
            if self.nodes[index].is_popped_at(self.current_snapshot()) {
                ...
                // end of the nodes array, so that we can always use the alive_len associated with
                // the snapshot (... which itself is not yet implemented).
                continue;
            }
            self.inherit_error(index);
            ...
            self.nodes[parent].snapshots.update_with(
                ...
                |_, state| match state {
                    &NodeState::Success { ref num_incomplete_children, ref obligation } => {
                        ...
                        Some(NodeState::Success { num_incomplete_children: num_incomplete_children - 1,
    ...
    fn compress(&mut self) -> Vec<O> {
        // FIXME take advantage of the implicit ordering of roots introduced in snapshots in
        // self.nodes (i.e. use the latest snapshot's alive_len and the second latest snapshot's
        // alive_len to iterate over fewer nodes).
                ...
                if stack.is_empty() {
                    ...
                    let mut do_push = false;
                    {
                        ...
                        if potential_overwrite.snapshot == into_snapshot {
                            potential_overwrite.state = mem::replace(&mut to_commit.state, NodeState::Popped);
                        } else {
                            do_push = true;
                        }
                    }
                    if do_push {
                        stack.push(NodeStateSnapshot::new(
                                into_snapshot, mem::replace(&mut to_commit.state, NodeState::Popped)));
                    }
                }
            },
            &mut NodeStateSnapshots::Base(ref mut base) => {
        ...
        match self {
            &mut NodeStateSnapshots::Stack(ref mut stack) => {
                ...
                match snapshot_cmp {
                    ...
                    Ordering::Less => {
                        stack.push(NodeStateSnapshot::new(snapshot, new_state.unwrap()));
                    },
                    Ordering::Greater => panic!("failure to maintain stack discipline")
                }
            },
...
impl<O> Node<O> {
    ...
    // FIXME rename this to not be confusing; there's a state Popped that isn't directly synonymous
    // with this method.
    /// Whether or not we're done using this node at the given snapshot.
